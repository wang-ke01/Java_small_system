...
public class QuanManager extends JInternalFrame {
	...
	public QuanManager() {
		...
		setupComponet(userName, 3, 0, 1, 100, true);
		...
		modifyButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				TbUserlist user = DaoRu.getUser(item);
				int index = quanXian.getSelectedIndex();
				if (index == 0)
					user.setQuan("a");
				else
					user.setQuan("c");
				if (DaoRu.updateUser(user) >= 1)
	...
	private void doUserSelectAction() {
		...
		TbUserlist user = DaoRu.getUser(selectedItem);
