...
public class XiaoShouChaXun extends JInternalFrame {
	...
	public XiaoShouChaXun() {
		...
		condition.setModel(new DefaultComboBoxModel(new String[] { "客户全称",
				"销售票号" }));
	...
	class ShowAllActoinListener implements ActionListener { // 显示全部按钮的动作监听器
		public void actionPerformed(final ActionEvent e) {
			content.setText("");
			List list = DaoRu.findForList("select * from v_sellView");
	...
	class QueryActionListener implements ActionListener {
		public void actionPerformed(final ActionEvent e) {
			...
			List list = null;
			String con = condition.getSelectedIndex() == 0 ? "khname "
					: "sellId ";
			int oper = operation.getSelectedIndex();
			String opstr = oper == 0 ? "= " : "like ";
			String cont = content.getText();
			list = DaoRu.findForList("select * from v_sellView where "
					+ con
					+ opstr
					+ (oper == 0 ? "'" + cont + "'" : "'%" + cont + "%'")
					+ (selDate ? " and xsdate>'" + startDate.getText()
							+ "' and xsdate<='" + endDate.getText()
							+ " 23:59:59'" : ""));
			Iterator iterator = list.iterator();
			updateTable(iterator);
		}
