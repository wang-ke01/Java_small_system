...
public class XiaoShouDan extends JInternalFrame {
	...
	private final JComboBox jsfs = new JComboBox(); // 结算算方式
	...
	private void initSpBox() {
		...
		ResultSet set = DaoRu.query(" select * from tb_spinfo"
	...
	private void initPiaoHao() {
		...
		java.sql.Date date;
		try {
			date = new java.sql.Date(xssjDate.getTime());
			String maxId = DaoRu.getSellMainMaxId(date);
