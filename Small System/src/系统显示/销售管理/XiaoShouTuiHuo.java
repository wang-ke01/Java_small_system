...
public class XiaoShouTuiHuo extends JInternalFrame {
		...
		thButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (table.getRowCount() <= 0) {
					JOptionPane
							.showMessageDialog(XiaoShouTuiHuo.this, "添加退货商品");
					return;
				}
				...
				boolean rs = DaoRu.insertXsthInfo(xsthMain);
				if (rs) {
	...
	private void initSpBox() {
		...
		ResultSet set = DaoRu
				.query("select * from tb_spinfo where id in (select spid from v_sellView where khname = "
						+ "'" + khInfo.getCustomerName() + "')");
		sp.removeAllItems();
	...
	private final class initTasks extends InternalFrameAdapter {
		...
		private void initKehuField() {// 初始化客户字段
			List gysInfos = DaoRu.getKhInfos();
	...
	private void initPiaoHao() {
		java.sql.Date date = new java.sql.Date(thsjDate.getTime());
		String maxId = DaoRu.getXsthMainMaxId(date);
	...
	private synchronized void updateTable() {
		...
		TbKucun kucun = DaoRu.getKucun(item);
